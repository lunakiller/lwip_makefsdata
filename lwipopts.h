...
#define HTTPD_SERVER_AGENT "STM32H7, lwIP/2.1.2 (http://savannah.nongnu.org/projects/lwip)"
...
extern void UART_debug(int len, char* s);
#define UART_debugF(...) {\
		char buffer[128];\
		int len = 0;\
		len = snprintf(buffer, 128, __VA_ARGS__);\
		UART_debug(len, buffer);\
}

#ifdef LWIP_PLATFORM_DIAG
#undef LWIP_PLATFORM_DIAG
#endif
#define LWIP_PLATFORM_DIAG(x) UART_debugF x
/* USER CODE END 1 */
